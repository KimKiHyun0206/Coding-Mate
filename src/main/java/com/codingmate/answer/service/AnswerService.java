...
import com.codingmate.exception.exception.answer.AnswerUpdateFailException;
...
public class AnswerService {
    ...
    public void update(String username, Long answerId, AnswerUpdateRequest request) {
        ...
        Answer answer = defaultAnswerRepository.findOneWithProgrammerById(answerId)
                .orElseThrow(() -> new NotFoundAnswerException(
                        ErrorMessage.NOT_FOUND_ANSWER,
                        String.format("요청한 풀이(%d)를 찾지 못했습니다.", answerId)
                ));
        ...
        if (!answer.getProgrammer().getLoginId().equals(username)) {
            throw new AnswerAndProgrammerDoNotMatchException(
                    ...
                    String.format("요청자(%s)는 풀이(%d)를 수정할 권한이 없습니다.", username, answerId)
            );
        }

        if (writeRepository.update(answerId, request) != 1) {
            throw new AnswerUpdateFailException(
                    ErrorMessage.ANSWER_UPDATE_FAIL,
                    String.format("풀이(%d)를 수정하는 데 실패했습니다.", answerId)
            );
        }
